...
class UnifiedRbiFormsTest extends TestCase
{
    ...
    public function test_saved_records_move_into_rbi_forms_without_copying_or_exposing_other_barangays(): void
    {
        ...
        $this->actingAs($staff)->get(route('barangay.rbi-updates.index'))->assertOk()
            ->assertSee('SavedPerson')->assertSee('UndatedPerson')->assertDontSee('OtherOfficePerson')
        ...
        $this->get(route('registry.index', ['source' => 'SAN ROQUE.xlsx', 'sheet' => 'new-inhabitants']))->assertRedirect(route('barangay.rbi-updates.index'));
