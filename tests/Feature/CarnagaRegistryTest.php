...
class CarnagaRegistryTest extends TestCase
{
    use RefreshDatabase;

    public function test_carnaga_has_workbook_pages_and_can_save_monthly_families(): void
    {
        $barangay = Barangay::where('name', 'Carnaga')->firstOrFail();
        $barangay->update(['secretary_name' => 'Carnaga Secretary', 'punong_barangay_name' => 'Carnaga Chair']);
        ...
        Inhabitant::create([
            ...
            'last_name' => 'Example', 'first_name' => 'CarnagaResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('CARNAGA')->assertSee('CarnagaResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('CARNAGA')->assertSee('Save Monthly Report')
            ->assertSee('Carnaga Secretary')->assertSee('Carnaga Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('CarnagaResident');
