...
class PunongRegistryTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_punong_has_workbook_pages_and_can_save_monthly_families(): void
    {
        $barangay = Barangay::where('name', 'Punong')->firstOrFail();
        $barangay->update(['secretary_name' => 'Punong Secretary', 'punong_barangay_name' => 'Punong Chair']);
        $secretary = User::factory()->create(['role' => User::ROLE_BARANGAY, 'barangay_id' => $barangay->id]);
        $household = Household::create(['barangay_id' => $barangay->id, 'household_number' => '1']);
        Inhabitant::create([
            'barangay_id' => $barangay->id, 'household_id' => $household->id,
            'last_name' => 'Example', 'first_name' => 'PunongResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('PUNONG')->assertSee('PunongResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('PUNONG')->assertSee('Save Monthly Report')
            ->assertSee('Punong Secretary')->assertSee('Punong Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('PunongResident');
