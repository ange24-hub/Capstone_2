...
class RizalRegistryTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_rizal_has_workbook_pages_and_can_save_monthly_families(): void
    {
        $barangay = Barangay::where('name', 'Rizal')->firstOrFail();
        $barangay->update(['secretary_name' => 'Rizal Secretary', 'punong_barangay_name' => 'Rizal Chair']);
        $secretary = User::factory()->create(['role' => User::ROLE_BARANGAY, 'barangay_id' => $barangay->id]);
        $household = Household::create(['barangay_id' => $barangay->id, 'household_number' => '1']);
        Inhabitant::create([
            'barangay_id' => $barangay->id, 'household_id' => $household->id,
            'last_name' => 'Example', 'first_name' => 'RizalResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('RIZAL')->assertSee('RizalResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('RIZAL')->assertSee('Save Monthly Report')
            ->assertSee('Rizal Secretary')->assertSee('Rizal Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('RizalResident');
