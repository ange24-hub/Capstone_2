...
class SanAgustinRegistryTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_san_agustin_has_workbook_pages_and_can_save_monthly_families(): void
    {
        $barangay = Barangay::where('name', 'San Agustin')->firstOrFail();
        $barangay->update(['secretary_name' => 'SanAgustin Secretary', 'punong_barangay_name' => 'SanAgustin Chair']);
        $secretary = User::factory()->create(['role' => User::ROLE_BARANGAY, 'barangay_id' => $barangay->id]);
        $household = Household::create(['barangay_id' => $barangay->id, 'household_number' => '1']);
        Inhabitant::create([
            'barangay_id' => $barangay->id, 'household_id' => $household->id,
            'last_name' => 'Example', 'first_name' => 'SanAgustinResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('SAN AGUSTIN')->assertSee('SanAgustinResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('SAN AGUSTIN')->assertSee('Save Monthly Report')
            ->assertSee('SanAgustin Secretary')->assertSee('SanAgustin Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('SanAgustinResident');
