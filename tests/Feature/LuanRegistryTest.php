...
class LuanRegistryTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_luan_has_workbook_pages_and_can_save_monthly_families(): void
    {
        $barangay = Barangay::where('name', 'Luan')->firstOrFail();
        $barangay->update(['secretary_name' => 'Luan Secretary', 'punong_barangay_name' => 'Luan Chair']);
        $secretary = User::factory()->create(['role' => User::ROLE_BARANGAY, 'barangay_id' => $barangay->id]);
        $household = Household::create(['barangay_id' => $barangay->id, 'household_number' => '1']);
        Inhabitant::create([
            'barangay_id' => $barangay->id, 'household_id' => $household->id,
            'last_name' => 'Example', 'first_name' => 'LuanResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('LUAN')->assertSee('LuanResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('LUAN')->assertSee('Save Monthly Report')
            ->assertSee('Luan Secretary')->assertSee('Luan Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('LuanResident');
