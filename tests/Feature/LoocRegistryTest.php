...
class LoocRegistryTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_looc_has_workbook_pages_and_can_save_monthly_families(): void
    {
        $barangay = Barangay::where('name', 'Looc')->firstOrFail();
        $barangay->update(['secretary_name' => 'Looc Secretary', 'punong_barangay_name' => 'Looc Chair']);
        $secretary = User::factory()->create(['role' => User::ROLE_BARANGAY, 'barangay_id' => $barangay->id]);
        $household = Household::create(['barangay_id' => $barangay->id, 'household_number' => '1']);
        Inhabitant::create([
            'barangay_id' => $barangay->id, 'household_id' => $household->id,
            'last_name' => 'Example', 'first_name' => 'LoocResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('LOOC')->assertSee('LoocResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('LOOC')->assertSee('Save Monthly Report')
            ->assertSee('Looc Secretary')->assertSee('Looc Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('LoocResident');
