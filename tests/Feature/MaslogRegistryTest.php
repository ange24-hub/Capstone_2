...
class MaslogRegistryTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $barangay = Barangay::where('name', 'Maslog')->firstOrFail();
        $barangay->update(['secretary_name' => 'Maslog Secretary', 'punong_barangay_name' => 'Maslog Chair']);
        $secretary = User::factory()->create(['role' => User::ROLE_BARANGAY, 'barangay_id' => $barangay->id]);
        $household = Household::create(['barangay_id' => $barangay->id, 'household_number' => '1']);
        Inhabitant::create([
            'barangay_id' => $barangay->id, 'household_id' => $household->id,
            'last_name' => 'Example', 'first_name' => 'MaslogResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('MASLOG')->assertSee('MaslogResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('MASLOG')->assertSee('Save Monthly Report')
            ->assertSee('Maslog Secretary')->assertSee('Maslog Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('MaslogResident');
