...
class HinapoRegistryTest extends TestCase
{
    use RefreshDatabase;

    public function test_hinapo_has_workbook_pages_and_can_save_monthly_families(): void
    {
        $barangay = Barangay::where('name', 'Hinapo')->firstOrFail();
        $barangay->update(['secretary_name' => 'Hinapo Secretary', 'punong_barangay_name' => 'Hinapo Chair']);
        ...
        Inhabitant::create([
            ...
            'last_name' => 'Example', 'first_name' => 'HinapoResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('HINAPO')->assertSee('HinapoResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('HINAPO')->assertSee('Save Monthly Report')
            ->assertSee('Hinapo Secretary')->assertSee('Hinapo Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('HinapoResident');
