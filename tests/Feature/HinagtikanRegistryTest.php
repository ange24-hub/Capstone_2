...
class HinagtikanRegistryTest extends TestCase
{
    use RefreshDatabase;

    public function test_hinagtikan_has_workbook_pages_and_can_save_monthly_families(): void
    {
        $barangay = Barangay::where('name', 'Hinagtikan')->firstOrFail();
        $barangay->update(['secretary_name' => 'Hinagtikan Secretary', 'punong_barangay_name' => 'Hinagtikan Chair']);
        ...
        Inhabitant::create([
            ...
            'last_name' => 'Example', 'first_name' => 'HinagtikanResident',
        ...
        $this->get(route('barangay.registry.active'))->assertOk()
            ->assertSee('HINAGTIKAN')->assertSee('HinagtikanResident')->assertSee('Not provided');
        ...
        $this->get(route('barangay.registry.new-inhabitants'))->assertOk()
            ->assertSee('HINAGTIKAN')->assertSee('Save Monthly Report')
            ->assertSee('Hinagtikan Secretary')->assertSee('Hinagtikan Chair')
        ...
        $this->actingAs($otherSecretary)->get(route('barangay.registry.active'))
            ->assertOk()->assertDontSee('HinagtikanResident');
