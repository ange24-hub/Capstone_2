...
$source = dirname(__DIR__).'/storage/app/import-sources/SAN MIGUEL.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function sanMiguelWorkbook(string $path): array
{
    return \App\Support\ResidenceWorkbookReader::read($path, ['CONSOLIDATED RBI', 'DECEASED', 'NEW']);
}

function sanMiguelDate(?string $value): ?string
{
...
function sanMiguelAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function sanMiguelSex(?string $value): ?string
{
...
function sanMiguelMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) { $date = sanMiguelDate($value); return $date ? Carbon::parse($date)->startOfMonth()->toDateString() : null; }
...
$sheets = sanMiguelWorkbook($source);
// Only an explicit, non-negated transferred remark changes registry status.
$plan = \App\Support\SanMiguelWorkbook::plan($sheets);
...
$invalidDates = [];
function sanMiguelPerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = sanMiguelPerson($row, $columns);
...
$summary = [
    ...
    'residence_counts' => array_count_values(array_map(fn ($row) => \App\Support\SanMiguelWorkbook::residence($row)[0], $plan['active'])),
    'missing_household_numbers' => array_values(array_diff(range(1, 128), array_keys($plan['households']))),
...
foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
    if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('San Miguel already has records; refusing to overwrite or duplicate them.');
}
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active SanMiguel records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/SAN MIGUEL-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/SAN MIGUEL-review-'.now()->format('Ymd-His').'.csv';
...
foreach ($plan['active'] as $position => $row) {
    if (\App\Support\SanMiguelWorkbook::residence($row)[0] === Inhabitant::RESIDENCE_UNCONFIRMED) fputcsv($review, [$position, $row['B'], $row['C'], $row['D'], \App\Support\SanMiguelWorkbook::residence($row)[1]], ',', '"', '');
...
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('SanMiguel already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach (['active', 'moved'] as $group) {
        foreach ($prepared[$group] ?? [] as $position => $data) {
            ...
            Inhabitant::create($data + [
                ...
                'residence_status'=>\App\Support\SanMiguelWorkbook::residence($row)[0],
                'residence_source'=>'SAN MIGUEL.xlsx '.$row['_source'].': '.\App\Support\SanMiguelWorkbook::residence($row)[1],
            ]);
        }
    }
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>sanMiguelMonth($month)]);
    }
});
