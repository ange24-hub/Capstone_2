...
$source = dirname(__DIR__).'/storage/app/import-sources/SAN ANTONIO.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function sanAntonioWorkbook(string $path): array
{
    return \App\Support\ResidenceWorkbookReader::read($path, ['CONSOLIDATED RBI', 'DECEASED', 'NEW', 'OUT']);
}

function sanAntonioDate(?string $value): ?string
{
...
function sanAntonioAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function sanAntonioSex(?string $value): ?string
{
...
function sanAntonioMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) { $date = sanAntonioDate($value); return $date ? Carbon::parse($date)->startOfMonth()->toDateString() : null; }
...
$sheets = sanAntonioWorkbook($source);
// Only an explicit destination-bearing transfer remark changes residency status.
$plan = \App\Support\SanAntonioWorkbook::plan($sheets);
...
$invalidDates = [];
function sanAntonioPerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = sanAntonioPerson($row, $columns);
...
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active SanAntonio records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/SAN ANTONIO-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/SAN ANTONIO-review-'.now()->format('Ymd-His').'.csv';
...
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('SanAntonio already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach (['active', 'moved'] as $group) {
        foreach ($prepared[$group] ?? [] as $position => $data) {
            ...
            Inhabitant::create($data + [
                ...
                'residence_status'=>\App\Support\SanAntonioWorkbook::residence($row)[0],
                'residence_source'=>'SAN ANTONIO.xlsx '.$row['_source'].': '.\App\Support\SanAntonioWorkbook::residence($row)[1],
            ]);
        }
    }
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>sanAntonioMonth($month)]);
    }
});
