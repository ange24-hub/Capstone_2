...
$source = dirname(__DIR__).'/storage/app/import-sources/MASLOG.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function maslogWorkbook(string $path): array
{
    return \App\Support\ResidenceWorkbookReader::read($path, ['CONSOLIDATED RBI']);
}

function maslogDate(?string $value): ?string
{
...
function maslogAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function maslogSex(?string $value): ?string
{
...
function maslogMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) return Carbon::parse(maslogDate($value))->startOfMonth()->toDateString();
...
$sheets = maslogWorkbook($source);
$emeliaHousehold = null;
foreach ($argv as $argument) if (str_starts_with($argument, '--emelia-household=')) $emeliaHousehold = substr($argument, strlen('--emelia-household='));
$plan = \App\Support\MaslogWorkbook::plan($sheets, $emeliaHousehold);
$barangay = Barangay::where('name', 'Maslog')->firstOrFail();
$counts = fn () => [
...
$invalidDates = [];
function maslogPerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = maslogPerson($row, $columns);
...
$summary = [
    'residence_rule' => 'Place remarks remain registered, living elsewhere; deceased markings in P/Q/R are separate',
    'emelia_household' => $emeliaHousehold,
    'source_sha256' => hash_file('sha256', $source),
    'source_counts' => array_map('count', $plan),
    'missing_household_numbers' => array_values(array_diff(range(1, 230), array_keys($plan['households']))),
...
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active Maslog records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/MASLOG-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/MASLOG-review-'.now()->format('Ymd-His').'.csv';
...
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('Maslog already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>maslogMonth($month)]);
    }
});
