...
$source = dirname(__DIR__).'/storage/app/import-sources/MAG-ATA.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function magataWorkbook(string $path): array
{
    $zip = new ZipArchive();
...
function magataDate(?string $value): ?string
{
...
function magataAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function magataSex(?string $value): ?string
{
...
function magataMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) return Carbon::parse(magataDate($value))->startOfMonth()->toDateString();
...
$sheets = magataWorkbook($source);
// Only an explicit destination-bearing transfer remark changes residency status.
$vecinaResolution = in_array('--merge-vecina', $argv, true) ? 'merge' : (in_array('--distinct-vecina', $argv, true) ? 'distinct' : 'review');
$plan = \App\Support\MagataWorkbook::plan($sheets, $vecinaResolution);
...
$invalidDates = [];
function magataPerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = magataPerson($row, $columns);
...
$summary = [
    'moved_out_rule' => 'Explicit TRANSFER destination; color alone does not change status',
    'vecina_resolution' => $vecinaResolution,
    'source_sha256' => hash_file('sha256', $source),
    'source_counts' => array_map('count', $plan),
    'missing_household_numbers' => array_values(array_diff(range(1, 101), array_keys($plan['households']))),
...
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active Magata records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/MAG-ATA-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/MAG-ATA-review-'.now()->format('Ymd-His').'.csv';
...
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('Magata already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>magataMonth($month)]);
    }
});
