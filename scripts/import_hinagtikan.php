...
$source = dirname(__DIR__).'/storage/app/import-sources/HINAGTIKAN.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function hinagtikanWorkbook(string $path): array
{
    $zip = new ZipArchive();
...
function hinagtikanDate(?string $value): ?string
{
...
function hinagtikanSex(?string $value): ?string
{
...
function hinagtikanMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) return Carbon::parse(hinagtikanDate($value))->startOfMonth()->toDateString();
...
$sheets = hinagtikanWorkbook($source);
$barangay = Barangay::where('name', 'Hinagtikan')->firstOrFail();
if (($sheets['CONSOLIDATED RBI'][3]['O'] ?? null) !== 'HINAGTIKAN') {
    throw new RuntimeException('The consolidated worksheet is not for Hinagtikan.');
}
// These members continue the numbered household immediately above in the workbook.
// Rows 86–87 start a separate family without a household number and remain pending.
foreach ([85 => '73', 99 => '22', 137 => '30', 138 => '30', 169 => '35'] as $position => $number) {
...
if (! isset($sheets['CONSOLIDATED RBI'], $sheets['DECEASED'], $sheets['NEW']) || $activeRows->isEmpty()) {
    throw new RuntimeException('The expected Hinagtikan worksheets or active records are missing.');
}
if (in_array('--dry-run', $argv, true)) {
    echo json_encode([
        ...
        'rows_with_unconfirmed_sex' => $activeRows->filter(fn ($row) => hinagtikanSex($row['L'] ?? null) === null)->keys()->all(),
        'invalid_active_birth_date_rows' => $activeRows->filter(fn ($row) => filled($row['J'] ?? null) && hinagtikanDate($row['J']) === null)->keys()->all(),
...
$backupPath = $backupDirectory.'/HINAGTIKAN-before-'.now()->format('Ymd-His').'.json';
...
DB::transaction(function () use ($barangay, $activeRows, $deceasedRows, $newRows, $sourceHouseholds): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) {
            throw new RuntimeException('Hinagtikan already has records; refusing to overwrite or duplicate them.');
        }
    }
    ...
    foreach ($activeRows as $position => $row) {
        ...
        Inhabitant::create([
            ...
            'birth_place' => $row['I'] ?? null, 'birth_date' => hinagtikanDate($row['J'] ?? null),
            'recorded_age' => is_numeric($row['K'] ?? null) ? (int) $row['K'] : null,
            'sex' => hinagtikanSex($row['L'] ?? null) ?? '', 'civil_status' => $row['M'] ?? null,
            'education_level' => $row['N'] ?? null, 'religion' => $row['O'] ?? null,
            'occupation' => $row['P'] ?? null,
            'remarks' => trim(($row['Q'] ?? '')
                .(filled($row['J'] ?? null) && hinagtikanDate($row['J']) === null ? ' [Source birth date: '.$row['J'].']' : '')
                .(hinagtikanSex($row['L'] ?? null) === null ? ' [Source sex: '.(($row['L'] ?? '') ?: 'not provided').']' : '')) ?: null,
    ...
    foreach ($deceasedRows as $position => $row) {
        DeceasedInhabitant::create([
            ...
            'birth_date' => hinagtikanDate($row['I'] ?? null), 'recorded_age' => is_numeric($row['J'] ?? null) ? (int) $row['J'] : null,
            'sex' => hinagtikanSex($row['K'] ?? null), 'civil_status' => $row['L'] ?? null,
            'education_level' => $row['M'] ?? null, 'religion' => $row['N'] ?? null,
            'occupation' => $row['O'] ?? null, 'remarks' => null,
            'death_date' => hinagtikanDate($row['P'] ?? null), 'source_position' => $position,
        ]);
    }
    ...
    foreach ($newRows as $position => $row) {
        NewInhabitant::create([
            'barangay_id' => $barangay->id, 'household_number' => $row['A'],
            'last_name' => $row['B'], 'first_name' => $row['C'], 'middle_name' => $row['D'] ?? null,
            'suffix' => $row['E'] ?? null, 'birth_date' => hinagtikanDate($row['F'] ?? null),
            'recorded_age' => is_numeric($row['G'] ?? null) ? (int) $row['G'] : null,
            'sex' => hinagtikanSex($row['H'] ?? null), 'civil_status' => $row['I'] ?? null,
            ...
            'month_submitted' => $row['N'] ?? null, 'reporting_month' => hinagtikanMonth($row['N'] ?? null),
