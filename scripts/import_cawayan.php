...
$source = dirname(__DIR__).'/storage/app/import-sources/CAWAYAN.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function cawayanWorkbook(string $path): array
{
    $zip = new ZipArchive();
...
function cawayanDate(?string $value): ?string
{
...
function cawayanSex(?string $value): ?string
{
...
function cawayanMonth(?string $value): ?string
{
...
$sheets = cawayanWorkbook($source);
$barangay = Barangay::where('name', 'Cawayan')->firstOrFail();
if (($sheets['CONSOLIDATED RBI'][3]['O'] ?? null) !== 'CAWAYAN') {
    throw new RuntimeException('The consolidated worksheet is not for Cawayan.');
}
// The unnumbered daughter at row 833 continues household 212 immediately above.
if (blank($sheets['CONSOLIDATED RBI'][833]['B'] ?? null)
    && ($sheets['CONSOLIDATED RBI'][832]['B'] ?? null) === '212'
    && ($sheets['CONSOLIDATED RBI'][833]['G'] ?? null) === 'Daughter') {
    $sheets['CONSOLIDATED RBI'][833]['B'] = '212';
}
...
if (! isset($sheets['CONSOLIDATED RBI'], $sheets['DECEASED'], $sheets['NEW']) || $activeRows->isEmpty()) {
    throw new RuntimeException('The expected Cawayan worksheets or active records are missing.');
}
if (in_array('--dry-run', $argv, true)) {
    echo json_encode([
        ...
        'rows_with_unconfirmed_sex' => $activeRows->filter(fn ($row) => cawayanSex($row['L'] ?? null) === null)->keys()->all(),
        'invalid_active_birth_date_rows' => $activeRows->filter(fn ($row) => filled($row['J'] ?? null) && cawayanDate($row['J']) === null)->keys()->all(),
...
$backupPath = $backupDirectory.'/CAWAYAN-before-'.now()->format('Ymd-His').'.json';
...
DB::transaction(function () use ($barangay, $activeRows, $deceasedRows, $newRows, $sourceHouseholds): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) {
            throw new RuntimeException('Cawayan already has records; refusing to overwrite or duplicate them.');
        }
    }
    ...
    foreach ($activeRows as $position => $row) {
        ...
        Inhabitant::create([
            ...
            'birth_place' => $row['I'] ?? null, 'birth_date' => cawayanDate($row['J'] ?? null),
            'recorded_age' => is_numeric($row['K'] ?? null) ? (int) $row['K'] : null,
            'sex' => cawayanSex($row['L'] ?? null) ?? '', 'civil_status' => $row['M'] ?? null,
            'education_level' => $row['N'] ?? null, 'religion' => $row['O'] ?? null,
            'occupation' => $row['P'] ?? null,
            'remarks' => trim(($row['Q'] ?? '')
                .(filled($row['J'] ?? null) && cawayanDate($row['J']) === null ? ' [Source birth date: '.$row['J'].']' : '')
                .(cawayanSex($row['L'] ?? null) === null ? ' [Source sex: '.(($row['L'] ?? '') ?: 'not provided').']' : '')) ?: null,
    ...
    foreach ($deceasedRows as $position => $row) {
        DeceasedInhabitant::create([
            ...
            'birth_date' => cawayanDate($row['I'] ?? null), 'recorded_age' => is_numeric($row['J'] ?? null) ? (int) $row['J'] : null,
            'sex' => cawayanSex($row['K'] ?? null), 'civil_status' => $row['L'] ?? null,
            'education_level' => $row['M'] ?? null, 'religion' => $row['N'] ?? null,
            'occupation' => $row['O'] ?? null, 'remarks' => $row['P'] ?? null,
            'death_date' => cawayanDate($row['Q'] ?? null), 'source_position' => $position,
        ]);
    }
    ...
    foreach ($newRows as $position => $row) {
        NewInhabitant::create([
            'barangay_id' => $barangay->id, 'household_number' => $row['A'],
            'last_name' => $row['B'], 'first_name' => $row['C'], 'middle_name' => $row['D'] ?? null,
            'suffix' => $row['E'] ?? null, 'birth_date' => cawayanDate($row['F'] ?? null),
            'recorded_age' => is_numeric($row['G'] ?? null) ? (int) $row['G'] : null,
            'sex' => cawayanSex($row['H'] ?? null), 'civil_status' => $row['I'] ?? null,
            ...
            'month_submitted' => $row['N'] ?? null, 'reporting_month' => cawayanMonth($row['N'] ?? null),
