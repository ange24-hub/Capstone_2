...
$source = dirname(__DIR__).'/storage/app/import-sources/LUAN.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function luanWorkbook(string $path): array
{
    $zip = new ZipArchive();
...
function luanDate(?string $value): ?string
{
...
function luanAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function luanSex(?string $value): ?string
{
...
function luanMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) return Carbon::parse(luanDate($value))->startOfMonth()->toDateString();
...
$sheets = luanWorkbook($source);
// Only an explicit destination-bearing transfer remark changes residency status.
$plan = \App\Support\LuanWorkbook::plan($sheets);
$barangay = Barangay::where('name', 'Luan')->firstOrFail();
$counts = fn () => [
...
$invalidDates = [];
function luanPerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = luanPerson($row, $columns);
...
$summary = [
    ...
    'missing_household_numbers' => array_values(array_diff(range(1, (int) ($sheets['CONSOLIDATED RBI'][5]['P'] ?? 0)), array_keys($plan['households']))),
...
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active Luan records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/LUAN-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/LUAN-review-'.now()->format('Ymd-His').'.csv';
...
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('Luan already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>luanMonth($month)]);
    }
});
