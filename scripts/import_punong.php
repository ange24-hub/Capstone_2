...
$source = dirname(__DIR__).'/storage/app/import-sources/PUNONG.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function punongWorkbook(string $path): array
{
    return \App\Support\ResidenceWorkbookReader::read($path, ['CONSOLIDATED RBI', 'DECEASED', 'NEW']);
}

function punongDate(?string $value): ?string
{
...
function punongAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function punongSex(?string $value): ?string
{
...
function punongMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) return Carbon::parse(punongDate($value))->startOfMonth()->toDateString();
...
$sheets = punongWorkbook($source);
// Only an explicit destination-bearing transfer remark changes residency status.
$plan = \App\Support\PunongWorkbook::plan($sheets);
$barangay = Barangay::where('name', 'Punong')->firstOrFail();
$counts = fn () => [
...
$invalidDates = [];
function punongPerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
$activeColumns = ['last_name'=>'C','first_name'=>'D','middle_name'=>'E','suffix'=>'F','relationship_to_head'=>'G',
    ...
    'religion'=>'O','occupation'=>'Q','remarks'=>'R'];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = punongPerson($row, $columns);
...
$summary = [
    'moved_out_rule' => 'Yellow name cells or location remarks mean registered, living elsewhere; isolated highlights in rows 28 and 29 excluded',
    'source_sha256' => hash_file('sha256', $source),
    'source_counts' => array_map('count', $plan),
    'missing_household_numbers' => array_values(array_diff(range(1, 166), array_keys($plan['households']))),
...
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active Punong records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/PUNONG-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/PUNONG-review-'.now()->format('Ymd-His').'.csv';
...
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('Punong already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach (['active', 'moved'] as $group) {
        foreach ($prepared[$group] ?? [] as $position => $data) {
            ...
            Inhabitant::create($data + [
                ...
                'registry_sequence'=>is_numeric($row['T'] ?? null) ? $row['T'] : $position - 11, 'family_number'=>$row['B'] ?: null,
                'individual_number'=>null,
                'status'=>$group === 'active' ? Inhabitant::STATUS_ACTIVE : Inhabitant::STATUS_MIGRATED_OUT,
                'residence_status'=>\App\Support\PunongWorkbook::residence($row)[0],
                'residence_source'=>'PUNONG.xlsx '.$row['_source'].': '.\App\Support\PunongWorkbook::residence($row)[1],
            ]);
        }
    }
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>punongMonth($month)]);
    }
});
