...
$source = dirname(__DIR__).'/storage/app/import-sources/INIGUIHAN.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function iniguihanWorkbook(string $path): array
{
    $zip = new ZipArchive();
    ...
    foreach ($styles->fills->fill as $fill) {
        $fills[] = strtoupper((string) $fill->patternFill->fgColor['rgb']) === 'FFFF0000';
    }
    $redStyles = [];
    foreach ($styles->cellXfs->xf as $style) {
        $redStyles[] = $fills[(int) $style['fillId']] ?? false;
    }
    $sheets = [];
    foreach ($book->sheets->sheet as $sheet) {
        ...
        foreach ($sheetXml->sheetData->row as $row) {
            ...
            foreach ($row->c as $cell) {
                ...
                if ($column === 'A' && ($redStyles[(int) $cell['s']] ?? false)) {
                    $values['_red'] = true;
                }
            }
...
function iniguihanDate(?string $value): ?string
{
    return \App\Support\IniguihanWorkbook::date($value);
}

function iniguihanAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function iniguihanSex(?string $value): ?string
{
...
function iniguihanMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) return Carbon::parse(iniguihanDate($value))->startOfMonth()->toDateString();
...
$rawSheets = iniguihanWorkbook($source);
$sheets = \App\Support\IniguihanWorkbook::normalize($rawSheets);
$barangay = Barangay::where('name', 'Iniguihan')->firstOrFail();
if (($sheets['CONSOLIDATED RBI'][3]['O'] ?? null) !== 'INIGUIHAN') {
    throw new RuntimeException('The consolidated worksheet is not for Iniguihan.');
}
...
$sourceHouseholds = collect(array_keys($sheets['households']));
...
if (! isset($sheets['CONSOLIDATED RBI'], $sheets['DECEASED'], $sheets['NEW']) || $activeRows->isEmpty()) {
    throw new RuntimeException('The expected Iniguihan worksheets or active records are missing.');
}
if (in_array('--dry-run', $argv, true)) {
    echo json_encode([
        'sheet2_imported' => false,
        'sheet1_named_residents' => array_sum(array_column($sheets['households'], 'members')),
        ...
        'rows_with_unconfirmed_sex' => $activeRows->filter(fn ($row) => iniguihanSex($row['L'] ?? null) === null)->keys()->all(),
        'invalid_active_birth_date_rows' => $activeRows->filter(fn ($row) => filled($row['J'] ?? null) && iniguihanDate($row['J']) === null)->keys()->all(),
...
$backupPath = $backupDirectory.'/INIGUIHAN-before-'.now()->format('Ymd-His').'.json';
...
if (in_array('--reconcile-deceased', $argv, true)) {
    require __DIR__.'/reconcile_iniguihan_deceased.php';
    exit(0);
}

DB::transaction(function () use ($barangay, $activeRows, $deceasedRows, $newRows, $sourceHouseholds, $sheets): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) {
            throw new RuntimeException('Iniguihan already has records; refusing to overwrite or duplicate them.');
        }
    }
    ...
    foreach ($activeRows as $position => $row) {
        ...
        if (! isset($households[$number])) {
            $households[$number] = Household::create([
                ...
                'purok' => $row['H'] ?? null,
                'address' => $row['source_address'] ?? null,
            ]);
        }
        Inhabitant::create([
            'barangay_id' => $barangay->id,
            ...
            'birth_place' => $row['I'] ?? null, 'birth_date' => iniguihanDate($row['J'] ?? null),
            'recorded_age' => iniguihanAge($row['K'] ?? null),
            'sex' => iniguihanSex($row['L'] ?? null) ?? '', 'civil_status' => $row['M'] ?? null,
            'education_level' => $row['N'] ?? null, 'religion' => $row['O'] ?? null,
            'occupation' => $row['P'] ?? null,
            'remarks' => trim(($row['Q'] ?? '')
                .' [Source: INIGUIHAN.xlsx Sheet1 row '.$position.'; address: '.($row['source_address'] ?? '').']'
                .(filled($row['K'] ?? null) && iniguihanAge($row['K']) === null ? ' [Source age: '.$row['K'].']' : '')
                .(filled($row['J'] ?? null) && iniguihanDate($row['J']) === null ? ' [Source birth date: '.$row['J'].']' : '')
                .(iniguihanSex($row['L'] ?? null) === null ? ' [Source sex: '.(($row['L'] ?? '') ?: 'not provided').']' : '')) ?: null,
    ...
    foreach ($sourceHouseholds as $number) {
        Household::firstOrCreate(['barangay_id' => $barangay->id, 'household_number' => $number], [
            'purok' => $sheets['households'][$number]['purok'],
            'address' => $sheets['households'][$number]['address'],
        ]);
    }
    ...
    foreach ($deceasedRows as $position => $row) {
        DeceasedInhabitant::create([
            ...
            'birth_date' => iniguihanDate($row['I'] ?? null), 'recorded_age' => iniguihanAge($row['J'] ?? null),
            'sex' => iniguihanSex($row['K'] ?? null), 'civil_status' => $row['L'] ?? null,
            'education_level' => $row['M'] ?? null, 'religion' => $row['N'] ?? null,
            'occupation' => $row['O'] ?? null, 'remarks' => trim(($row['source_remarks'] ?? '').' [Source: INIGUIHAN.xlsx Sheet1 row '.$position.']'.(filled($row['J'] ?? null) && iniguihanAge($row['J']) === null ? ' [Source age: '.$row['J'].']' : '')) ?: null,
                'death_date' => iniguihanDate($row['P'] ?? null), 'source_position' => $position,
        ]);
    }
    ...
    foreach ($newRows as $position => $row) {
        NewInhabitant::create([
            'barangay_id' => $barangay->id, 'household_number' => $row['A'],
            'last_name' => $row['B'], 'first_name' => $row['C'], 'middle_name' => $row['D'] ?? null,
            'suffix' => $row['E'] ?? null, 'birth_date' => iniguihanDate($row['F'] ?? null),
            'recorded_age' => iniguihanAge($row['G'] ?? null),
            'sex' => iniguihanSex($row['H'] ?? null), 'civil_status' => $row['I'] ?? null,
            ...
            'month_submitted' => $row['N'] ?? null, 'reporting_month' => iniguihanMonth($row['N'] ?? null),
