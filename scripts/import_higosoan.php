...
$source = dirname(__DIR__).'/storage/app/import-sources/HIGOSOAN.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function higosoanWorkbook(string $path): array
{
    $zip = new ZipArchive();
...
function higosoanDate(?string $value): ?string
{
...
function higosoanSex(?string $value): ?string
{
...
function higosoanMonth(?string $value): ?string
{
...
$sheets = higosoanWorkbook($source);
$barangay = Barangay::where('name', 'Higosoan')->firstOrFail();
if (($sheets['CONSOLIDATED RBI'][3]['P'] ?? null) !== 'HIGOSOAN') throw new RuntimeException('Wrong barangay workbook.');
$normalizeRow = function (array $row): array {
    $normalized = [];
    foreach (range('A', 'S') as $column) $normalized[$column] = $row[chr(ord($column) + 1)] ?? '';
    $normalized['Q'] = trim(($row['R'] ?? '').' '.($row['S'] ?? ''));
    return $normalized;
};
$sheets['CONSOLIDATED RBI'] = array_map($normalizeRow, $sheets['CONSOLIDATED RBI']);
$movedRows = collect($sheets['MOVE OUT'] ?? [])->map($normalizeRow)
    ->filter(fn ($row) => filled($row['C']) && filled($row['D']));
$sheets['DECEASED'] = array_map(fn ($row) => [
    'A' => $row['A'] ?? '', 'B' => $row['B'] ?? '', 'C' => $row['C'] ?? '', 'D' => $row['D'] ?? '',
    'E' => $row['E'] ?? '', 'F' => $row['F'] ?? '', 'I' => $row['G'] ?? '', 'J' => $row['H'] ?? '',
    'K' => $row['I'] ?? '', 'L' => $row['J'] ?? '', 'Q' => $row['K'] ?? '',
], $sheets['DECEASED'] ?? []);
...
$existingCounts = [
    ...
    'active_inhabitants' => Inhabitant::where('barangay_id', $barangay->id)->where('status', Inhabitant::STATUS_ACTIVE)->count(),
...
if (! isset($sheets['CONSOLIDATED RBI'], $sheets['DECEASED'], $sheets['NEW']) || $activeRows->isEmpty()) {
    throw new RuntimeException('The expected Higosoan worksheets or active records are missing.');
}
if (in_array('--dry-run', $argv, true)) {
    echo json_encode([
        ...
        'source_moved_out' => $movedRows->count(),
        'rows_missing_household_number' => $unassignedRows,
        'moved_out_missing_household' => $movedRows->filter(fn ($row) => blank($row['B']))->keys()->all(),
...
$backupPath = $backupDirectory.'/HIGOSOAN-before-'.now()->format('Ymd-His').'.json';
...
DB::transaction(function () use ($barangay, $activeRows, $deceasedRows, $newRows, $movedRows): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) {
            throw new RuntimeException('Higosoan already has records; refusing to overwrite or duplicate them.');
        }
    }
    ...
    foreach ($activeRows as $position => $row) {
        ...
        Inhabitant::create([
            ...
            'birth_place' => $row['I'] ?? null, 'birth_date' => higosoanDate($row['J'] ?? null),
            'recorded_age' => is_numeric($row['K'] ?? null) ? (int) $row['K'] : null,
            'sex' => higosoanSex($row['L'] ?? null) ?? '', 'civil_status' => $row['M'] ?? null,
            'education_level' => $row['N'] ?? null, 'religion' => $row['O'] ?? null,
            'occupation' => $row['P'] ?? null,
            'remarks' => trim(($row['Q'] ?? '')
                .(filled($row['J'] ?? null) && higosoanDate($row['J']) === null ? ' [Source birth date: '.$row['J'].']' : '')
                .(higosoanSex($row['L'] ?? null) === null ? ' [Source sex: '.(($row['L'] ?? '') ?: 'not provided').']' : '')) ?: null,
            'status' => Inhabitant::STATUS_ACTIVE,
        ]);
    }

    foreach ($movedRows as $position => $row) {
        $number = $row['B'] ?: 'Not recorded';
        $household = Household::firstOrCreate(['barangay_id' => $barangay->id, 'household_number' => $number], ['purok' => $row['H'] ?: null, 'address' => $row['H'] ?: null]);
        $attributes = [
            'barangay_id' => $barangay->id,
        ...
        $person = Inhabitant::where('barangay_id', $barangay->id)
            ->where('last_name', $row['C'])->where('first_name', $row['D'])
            ->where('middle_name', $row['E'])->where('birth_date', higosoanDate($row['J'] ?? null))->first();
        if ($person) $person->update($attributes);
        else Inhabitant::create($attributes);
    }
    ...
    foreach ($deceasedRows as $position => $row) {
        DeceasedInhabitant::create([
            ...
            'birth_date' => higosoanDate($row['I'] ?? null), 'recorded_age' => is_numeric($row['J'] ?? null) ? (int) $row['J'] : null,
            'sex' => higosoanSex($row['K'] ?? null), 'civil_status' => $row['L'] ?? null,
            'education_level' => $row['M'] ?? null, 'religion' => $row['N'] ?? null,
            'occupation' => $row['O'] ?? null, 'remarks' => $row['P'] ?? null,
            'death_date' => higosoanDate($row['Q'] ?? null), 'source_position' => $position,
        ]);
    }
    ...
    foreach ($newRows as $position => $row) {
        NewInhabitant::create([
            'barangay_id' => $barangay->id, 'household_number' => $row['A'],
            'last_name' => $row['B'], 'first_name' => $row['C'], 'middle_name' => $row['D'] ?? null,
            'suffix' => $row['E'] ?? null, 'birth_date' => higosoanDate($row['F'] ?? null),
            'recorded_age' => is_numeric($row['G'] ?? null) ? (int) $row['G'] : null,
            'sex' => higosoanSex($row['H'] ?? null), 'civil_status' => $row['I'] ?? null,
            ...
            'month_submitted' => $row['N'] ?? null, 'reporting_month' => higosoanMonth($row['N'] ?? null),
