...
$source = dirname(__DIR__).'/storage/app/import-sources/SAN ISIDRO.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function sanIsidroWorkbook(string $path): array
{
    return \App\Support\ResidenceWorkbookReader::read($path, ['CONSOLIDATED RBI', 'DECEASED']);
}

function sanIsidroDate(?string $value): ?string
{
...
function sanIsidroAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function sanIsidroSex(?string $value): ?string
{
...
function sanIsidroMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) { $date = sanIsidroDate($value); return $date ? Carbon::parse($date)->startOfMonth()->toDateString() : null; }
...
$sheets = sanIsidroWorkbook($source);
// Only an explicit, non-negated transferred remark changes registry status.
$plan = \App\Support\SanIsidroWorkbook::plan($sheets);
...
$invalidDates = [];
function sanIsidroPerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = sanIsidroPerson($row, $columns);
...
$summary = [
    'moved_out_rule' => 'OUT without explicit transferred remark remains active, living elsewhere; only explicit transfers are moved out',
    'source_sha256' => hash_file('sha256', $source),
    'source_counts' => array_map('count', $plan),
    'residence_counts' => array_count_values(array_map(fn ($row) => \App\Support\SanIsidroWorkbook::residence($row)[0], $plan['active'])),
    'missing_household_numbers' => array_values(array_diff(range(1, 347), array_keys($plan['households']))),
...
foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
    if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('San Isidro already has records; refusing to overwrite or duplicate them.');
}
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active SanIsidro records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/SAN ISIDRO-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/SAN ISIDRO-review-'.now()->format('Ymd-His').'.csv';
...
foreach ($plan['active'] as $position => $row) {
    if (\App\Support\SanIsidroWorkbook::residence($row)[0] === Inhabitant::RESIDENCE_UNCONFIRMED) fputcsv($review, [$position, $row['B'], $row['C'], $row['D'], \App\Support\SanIsidroWorkbook::residence($row)[1]], ',', '"', '');
...
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('SanIsidro already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach (['active', 'moved'] as $group) {
        foreach ($prepared[$group] ?? [] as $position => $data) {
            ...
            Inhabitant::create($data + [
                ...
                'residence_status'=>\App\Support\SanIsidroWorkbook::residence($row)[0],
                'residence_source'=>'SAN ISIDRO.xlsx '.$row['_source'].': '.\App\Support\SanIsidroWorkbook::residence($row)[1],
            ]);
        }
    }
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>sanIsidroMonth($month)]);
    }
});
