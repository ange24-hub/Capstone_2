...
function rizalAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function rizalSex(?string $value): ?string
{
...
function rizalMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) return Carbon::parse(rizalDate($value))->startOfMonth()->toDateString();
...
$sheets = rizalWorkbook($source);
// Only an explicit destination-bearing transfer remark changes residency status.
$plan = \App\Support\RizalWorkbook::plan($sheets);
$barangay = Barangay::where('name', 'Rizal')->firstOrFail();
$counts = fn () => [
...
$invalidDates = [];
function rizalPerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = rizalPerson($row, $columns);
...
$summary = [
    'moved_out_rule' => 'OFW or VACANT means registered, living elsewhere; VACANT interpretation confirmed by user',
    'source_sha256' => hash_file('sha256', $source),
    'source_counts' => array_map('count', $plan),
    'missing_household_numbers' => array_values(array_diff(range(1, 176), array_keys($plan['households']))),
...
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active Rizal records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/RIZAL-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/RIZAL-review-'.now()->format('Ymd-His').'.csv';
...
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('Rizal already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach (['active', 'moved'] as $group) {
        foreach ($prepared[$group] ?? [] as $position => $data) {
            ...
            Inhabitant::create($data + [
                ...
                'residence_status'=>\App\Support\RizalWorkbook::residence($row)[0],
                'residence_source'=>'RIZAL.xlsx '.$row['_source'].': '.\App\Support\RizalWorkbook::residence($row)[1],
            ]);
        }
    }
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>rizalMonth($month)]);
    }
});
