...
$source = dirname(__DIR__).'/storage/app/import-sources/CAMANSI.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function camansiWorkbook(string $path): array
{
    $zip = new ZipArchive();
...
function camansiDate(?string $value): ?string
{
...
function camansiSex(?string $value): ?string
{
...
function camansiMonth(?string $value): ?string
{
...
$sheets = camansiWorkbook($source);
$barangay = Barangay::where('name', 'Camansi')->firstOrFail();
...
if (! isset($sheets['CONSOLIDATED RBI'], $sheets['DECEASED'], $sheets['NEW']) || $activeRows->isEmpty()) {
    throw new RuntimeException('The expected Camansi worksheets or active records are missing.');
}
...
$backupPath = $backupDirectory.'/CAMANSI-before-'.now()->format('Ymd-His').'.json';
...
DB::transaction(function () use ($barangay, $activeRows, $deceasedRows, $newRows): void {
    ...
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) {
            throw new RuntimeException('Camansi already has records; refusing to overwrite or duplicate them.');
        }
    }
    ...
    foreach ($activeRows as $position => $row) {
        ...
        Inhabitant::create([
            ...
            'birth_place' => $row['I'] ?? null, 'birth_date' => camansiDate($row['J'] ?? null),
            'recorded_age' => is_numeric($row['K'] ?? null) ? (int) $row['K'] : null,
            'sex' => camansiSex($row['L'] ?? null), 'civil_status' => $row['M'] ?? null,
    ...
    foreach ($deceasedRows as $position => $row) {
        DeceasedInhabitant::create([
            ...
            'birth_date' => camansiDate($row['I'] ?? null), 'recorded_age' => is_numeric($row['J'] ?? null) ? (int) $row['J'] : null,
            'sex' => camansiSex($row['K'] ?? null), 'civil_status' => $row['L'] ?? null,
            'education_level' => $row['M'] ?? null, 'religion' => $row['N'] ?? null,
            'occupation' => $row['O'] ?? null, 'remarks' => $row['P'] ?? null,
            'death_date' => camansiDate($row['Q'] ?? null), 'source_position' => $position,
        ]);
    }
    ...
    foreach ($newRows as $position => $row) {
        NewInhabitant::create([
            'barangay_id' => $barangay->id, 'household_number' => $row['A'],
            'last_name' => $row['B'], 'first_name' => $row['C'], 'middle_name' => $row['D'] ?? null,
            'suffix' => $row['E'] ?? null, 'birth_date' => camansiDate($row['F'] ?? null),
            'recorded_age' => is_numeric($row['G'] ?? null) ? (int) $row['G'] : null,
            'sex' => camansiSex($row['H'] ?? null), 'civil_status' => $row['I'] ?? null,
            ...
            'month_submitted' => $row['N'] ?? null, 'reporting_month' => camansiMonth($row['N'] ?? null),
