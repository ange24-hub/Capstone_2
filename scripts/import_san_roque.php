...
$source = dirname(__DIR__).'/storage/app/import-sources/SAN ROQUE.xlsx';
if (! is_file($source)) {
    throw new RuntimeException("Missing source workbook: {$source}");
}

function sanRoqueWorkbook(string $path): array
{
    return \App\Support\ResidenceWorkbookReader::read($path, ['CONSOLIDATED RBI', 'DECEASED', 'NEW']);
}

function sanRoqueDate(?string $value): ?string
{
    ...
    if (! preg_match('/\d{4}/', $value) || preg_match('/^[A-Za-z.]+\s+\d{4}$/', trim($value))) return null;
...
function sanRoqueAge(?string $value): ?int
{
    return is_numeric($value) && (float) $value >= 0 && (float) $value <= 150 ? (int) $value : null;
}

function sanRoqueSex(?string $value): ?string
{
...
function sanRoqueMonth(?string $value): ?string
{
    $value = trim((string) $value);
    if ($value === '') return null;
    if (is_numeric($value)) { $date = sanRoqueDate($value); return $date ? Carbon::parse($date)->startOfMonth()->toDateString() : null; }
...
$sheets = sanRoqueWorkbook($source);
// Only an explicit, non-negated transferred remark changes registry status.
$plan = \App\Support\SanRoqueWorkbook::plan($sheets);
...
$counts = fn () => [
    ...
    'living_elsewhere' => Inhabitant::where('barangay_id', $barangay->id)->where('status', Inhabitant::STATUS_ACTIVE)->where('residence_status', Inhabitant::RESIDENCE_ELSEWHERE)->count(),
    'living_here' => Inhabitant::where('barangay_id', $barangay->id)->where('status', Inhabitant::STATUS_ACTIVE)->where('residence_status', Inhabitant::RESIDENCE_HERE)->count(),
    'residence_unconfirmed' => Inhabitant::where('barangay_id', $barangay->id)->where('status', Inhabitant::STATUS_ACTIVE)->where('residence_status', Inhabitant::RESIDENCE_UNCONFIRMED)->count(),
...
$invalidDates = [];
function sanRoquePerson(array $row, array $columns): array
{
    global $invalidDates;
    $data = [];
...
foreach (['active', 'moved', 'deceased', 'new'] as $group) {
    ...
    foreach ($plan[$group] as $position => $row) {
        $prepared[$group][$position] = sanRoquePerson($row, $columns);
        if (in_array($group, ['active', 'moved'], true)) {
            $prepared[$group][$position]['sex'] ??= '';
            if (blank($row['B'] ?? null)) {
                $prepared[$group][$position]['remarks'] .= ' [Household assignment pending confirmation; source address: '.($row['H'] ?? '').']';
            }
        }
    }
}
$summary = [
    'moved_out_rule' => 'Explicit transfers, including household destinations and known spelling variants, are moved out as instructed; HH notes without transfer remain active',
    'source_sha256' => hash_file('sha256', $source),
    'source_counts' => array_map('count', $plan),
    'residence_counts' => array_count_values(array_map(fn ($row) => \App\Support\SanRoqueWorkbook::residence($row)[0], $plan['active'])),
    'missing_household_numbers' => array_values(array_diff(range(1, 111), array_keys($plan['households']))),
...
foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
    if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('San Roque already has records; refusing to overwrite or duplicate them.');
}
if (empty($plan['active']) || empty($plan['households'])) throw new RuntimeException('No active SanRoque records found.');
$backupDirectory = dirname(__DIR__).'/storage/app/import-backups';
if (! is_dir($backupDirectory)) mkdir($backupDirectory, 0775, true);
$backupPath = $backupDirectory.'/SAN ROQUE-before-'.now()->format('Ymd-His').'.json';
...
$reviewPath = $backupDirectory.'/SAN ROQUE-review-'.now()->format('Ymd-His').'.csv';
...
foreach ($plan['active'] as $position => $row) {
    if (\App\Support\SanRoqueWorkbook::residence($row)[0] === Inhabitant::RESIDENCE_UNCONFIRMED) fputcsv($review, [$position, $row['B'], $row['C'], $row['D'], \App\Support\SanRoqueWorkbook::residence($row)[1]], ',', '"', '');
    if (blank($row['B'] ?? null) && ! isset($plan['pending'][$position])) fputcsv($review, [$position, '', $row['C'], $row['D'], 'Imported as active; household number needs confirmation'], ',', '"', '');
}
foreach ($invalidDates as $issue) fputcsv($review, [$issue, '', '', '', 'Invalid source date retained in resident remarks'], ',', '"', '');
fclose($review);
DB::transaction(function () use ($barangay, $plan, $prepared): void {
    Barangay::whereKey($barangay->id)->lockForUpdate()->firstOrFail();
    foreach ([Household::class, Inhabitant::class, DeceasedInhabitant::class, NewInhabitant::class] as $model) {
        if ($model::where('barangay_id', $barangay->id)->exists()) throw new RuntimeException('SanRoque already has records; refusing to overwrite or duplicate them.');
    }
    $households = [];
    ...
    foreach (['active', 'moved'] as $group) {
        foreach ($prepared[$group] ?? [] as $position => $data) {
            ...
            Inhabitant::create($data + [
                ...
                'residence_status'=>\App\Support\SanRoqueWorkbook::residence($row)[0],
                'residence_source'=>'SAN ROQUE.xlsx '.$row['_source'].': '.\App\Support\SanRoqueWorkbook::residence($row)[1],
            ]);
        }
    }
    ...
    foreach ($prepared['new'] ?? [] as $position => $data) {
        ...
        NewInhabitant::create($data + ['barangay_id'=>$barangay->id,'source_position'=>$position,
            'month_submitted'=>$month,'reporting_month'=>sanRoqueMonth($month)]);
    }
});
