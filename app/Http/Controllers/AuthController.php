...
class AuthController extends Controller
{
    ...
    public function register(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'role' => ['required', 'in:resident,barangay,municipal_lgu'],
            'access_code' => ['nullable', 'string', 'max:100'],
            'password' => ['required', 'confirmed', Password::defaults()],
        ]);

        $this->validateAccessCode($request, $validated['role']);
    ...
    private function validateAccessCode(Request $request, string $role): void
    {
        ...
        if (! $configuredCode || ! hash_equals($configuredCode, (string) $request->input('access_code'))) {
            throw ValidationException::withMessages([
                'access_code' => 'A valid LGU access code is required for this role.',
            ]);
        }
    }
}
