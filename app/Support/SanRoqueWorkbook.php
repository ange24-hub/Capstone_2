...
class SanRoqueWorkbook
{
    ...
    public static function residence(array $row): array
    {
        ...
        if (preg_match('/at blue oasis/i', $remark)) return [Inhabitant::RESIDENCE_UNCONFIRMED, 'Workplace or program location requires residence confirmation'];
    ...
    {
        if (($sheets['CONSOLIDATED RBI'][3]['O'] ?? '') !== 'SAN ROQUE') throw new RuntimeException('Expected the San Roque workbook.');
        ...
        foreach ($people as $position => $row) {
            if (! self::transferred($row) && ($target = self::localHousehold($row)) !== null) {
                ...
                if ($row['B'] !== $target) {
                    $pending['household-'.$position] = ['row' => $row, 'reason' => 'Assigned to source destination household '.$target.' within San Roque'];
        ...
        foreach ($sheets['NEW'] as $position => $row) {
            ...
            // This sheet omits education: J religion, K occupation, L remarks, M submission month.
            $original = $row;
            $row['J'] = '';
            $row['K'] = $original['J'] ?? '';
            $row['L'] = $original['K'] ?? '';
