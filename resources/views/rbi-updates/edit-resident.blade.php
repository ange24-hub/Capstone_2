...
<section class="dashboard-page grid gap-6">
    <header class="dashboard-page-header flex flex-wrap items-start justify-between gap-4">
            <x-workspace-heading icon="users"><span class="dashboard-eyebrow">RBI Forms &middot; {{ $resident->barangay->name }}</span><h1>Edit registered resident</h1><p>{{ $resident->fullName() }} &middot; Household {{ $resident->household?->household_number }}</p></x-workspace-heading><a class="button secondary-button" href="{{ route('barangay.registry.active') }}">Back to Consolidated</a></header>
